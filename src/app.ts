...
export class App {
          ...
          <div class="bar-actions">
            ...
            <details class="export-menu" id="export-menu">
              ...
              <div class="menu" role="group" aria-label="Export formats">
                <button type="button" data-export="png" aria-label="Export standard-resolution PNG map">PNG map <small>Standard resolution</small></button>
                <button type="button" data-export="svg" aria-label="Export editable SVG map">SVG map <small>Editable vector</small></button>
                <button type="button" data-export="json" aria-label="Export preset JSON">Preset JSON <small>Share or restore</small></button>
                <button type="button" data-export="csv" aria-label="Export emitter CSV">Emitter CSV <small>Engine handoff</small></button>
                <button type="button" data-export="png4" class="pro-export" aria-label="Export four-times-resolution PNG map, Pro feature">4× PNG <small>Pro · print-ready</small></button>
              </div>
            </details>
          </div>
        </section>
  ...
  private renderInspector(): void {
    ...
    if (!selected) {
      target.innerHTML = `<div class="inspector-heading"><div><p class="eyebrow">Emitters</p><h2>${this.project.emitters.length || 'None placed'}</h2></div>${this.license.unlocked ? '<button class="text-button" id="snapshots-button" type="button">Snapshots</button>' : ''}</div>${this.project.emitters.length ? `<ol class="emitter-list">${this.project.emitters.map((e) => `<li><button type="button" data-select="${e.id}"><svg class="color-dot" viewBox="0 0 10 10" aria-hidden="true" focusable="false"><circle cx="5" cy="5" r="5" fill="${escapeHtml(e.color)}" /></svg><span><strong>${escapeHtml(e.name)}</strong><small>${format(e.x)}, ${format(e.y)} · ${format(e.maxDistance)} ${escapeHtml(this.project.unit)}</small></span><span aria-hidden="true">›</span></button></li>`).join('')}</ol><p class="inspector-hint">Select a range to tune it. All curve names describe this map only; transfer values according to your engine.</p>` : `<div class="inspector-empty"><span aria-hidden="true">⌖</span><p>Emitter controls appear here after you place one.</p></div>`}`;
